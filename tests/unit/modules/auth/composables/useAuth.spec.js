...
describe('Pruebas en useAuth', () => { 
    ...
    test('createUser Exitoso', async() => { 
        ...
        const newUser = { name: 'Nelson', email: 'nelson@gmail.com'}

        mockStore.dispatch.mockReturnValue({ ok: true })

        const resp = await createUser(newUser)
        
        expect( mockStore.dispatch ).toHaveBeenCalledWith("auth/createUser", {"email": "nelson@gmail.com", "name": "Nelson"})
     ...
     test('createUser fallido, porque el usuario ya existe', async () => { 
      ...
      test('login existoso', async () => { 
        ...
        const loginForm = { email: 'nelson@gmail.com', password: '123456' }
    ...
    test('Computed: authState, username', () => { 
        ...
        expect( username.value).toBe('Nelson')
